...
import { queryPageApi, queryAllTypeApi, addApi, queryInfoApi, updateApi, deleteApi, registerApi } from '@/api/act'
...
const register = async (id) => {
  ElMessageBox.confirm('确定报名此活动吗?', '提示',
    { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' }
  ).then(async () => {// 确定
    regist.value.id = id
    let result = await registerApi(regist.value)
    if (result.code) {
...
<template>
  <el-text type="primary" size="large">活动报名</el-text>
  ...
  <!-- 功能按钮 -->
  <div class="container">
    <el-button type="primary" @click="addAct">+ 创建活动</el-button>
  </div>
    ...
    <el-table :data="actList" border style="width: 100%" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center">
        <template #default="scope">
          ...
          <el-button type="primary" size="mini" @click="register(scope.row.id)"><el-icon>
              <EditPen />
